 ...
 public class JpdlModelDrawer
 {
   ...
   public List<AnchorArea> getMaps(JpdlModel jpdlModel) {
     ...
     for (Node node : nodes.values()) {
       ...
       this.links.add(new AnchorArea(Integer.valueOf(x), Integer.valueOf(y), Integer.valueOf(x + w), Integer.valueOf(y + h), name, type));
     }
...
//         int cy;
   ...
   private void drawArrow(Graphics2D g2, int x1, int y1, int x2, int y2)
   {
     ...
     double xp3 = x2 + 2.0D * Math.cos(slopy);
     double yp3 = y2 + 2.0D * Math.sin(slopy);
     ...
     double a = 8.0D * siny; double b = 8.0D * cosy;
     double c = 4.0D * siny; double d = 4.0D * cosy;
     ...
     throws IOException
   {
     for (Node node : nodes.values()) {
       ...
       if (nodeInfos.get(node.getType()) != null) {
         ...
         this.links.add(new AnchorArea(Integer.valueOf(x), Integer.valueOf(y), Integer.valueOf(x + w), Integer.valueOf(y + h), name, node.getType()));
         ...
         int yLabel = (int)(node.getY() + (node.getHeight() - r2.getHeight()) / 2.0D - r2.getY());
         g2.setStroke(DEFAULT_LINE_STROKE);
