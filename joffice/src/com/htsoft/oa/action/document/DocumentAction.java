 ...
 public class DocumentAction extends BaseAction
 {
   ...
   public String save()
   {
     ...
     if (this.document.getDocId() == null) {
       ...
       doc.setAuthor(this.document.getAuthor());
       doc.setKeywords(this.document.getKeywords());
       doc.setDocFolder(this.document.getDocFolder());
   ...
   public String saveOnline()
   {
     ...
     if (StringUtils.isNotEmpty(fileId)) {
       ...
       if (this.document.getDocId() == null) {
         ...
         Document orgDocument = (Document)this.documentService.get(this.document.getDocId());
         orgDocument.setDocName(this.document.getDocName());
         orgDocument.setUpdatetime(new Date());
         orgDocument.setDocFolder(this.document.getDocFolder());
         this.documentService.save(orgDocument);
       }
       setJsonString("{success:true}");
     } else {
   ...
   {
     QueryFilter filter = new QueryFilter(getRequest());
     filter.addFilter("Q_isShared_SN_EQ", Document.ONLINE_DOCUMENT.toString());
     List<Document> list = this.documentService.getAll(filter);
     List lists = new ArrayList();
     for (Document doc : list) {
       DocumentFile file = new DocumentFile();
       Set filed = doc.getAttachFiles();
       FileAttach afile = (FileAttach)filed.iterator().next();
