   ...
   public String list()
   {
     ...
     for (MailBox mailBoxTemp : list) {
       ...
       buff.append("{boxId:'")
         ...
         .append("',content:'");
       String content = StringUtil.html2Text(mailTemp.getContent().replace("&nbsp;", ""));
...
//       String id;
       ...
       int id1 = recipientIDs.length; 
				for (int i = 0; i < id1; i++) {
				  String id = recipientIDs[i];
         if (!"".equals(id)) {
       ...
       int len = recipientIDs.length; 
				for (int sdf = 0; sdf < len; sdf++) {
   ...
   public String opt()
   {
     ...
     if (opt.equals("回复")) {
       ...
       reply.setRecipientIDs(this.mail.getAppSender().getUserId().toString());
