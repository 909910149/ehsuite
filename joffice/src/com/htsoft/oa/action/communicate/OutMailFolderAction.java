...
import com.htsoft.oa.service.communicate.OutMailService;
 ...
 public class OutMailFolderAction extends BaseAction
 {
   ...
   public String list()
   {
     ...
     List<OutMailFolder> outMailFolderList = this.outMailFolderService.getAllUserFolderByParentId(curUserId, Long.valueOf(FIRST_PARENTID));
   ...
   private String findChildsFolder(Long userId, Long parentId)
   {
     ...
     List<OutMailFolder> folders = this.outMailFolderService.getUserFolderByParentId(userId, parentId);
     if (folders.size() == 0) {
   ...
   public String get()
   {
     OutMailFolder outMailFolder = (OutMailFolder)this.outMailFolderService.get(this.folderId);
   ...
   public String save()
   {
     ...
     if ((parentId == null) || (parentId.longValue() == FIRST_PARENTID)) {
       ...
       parentFolder = (OutMailFolder)this.outMailFolderService.get(parentId);
       this.outMailFolder.setDepLevel(Integer.valueOf(parentFolder.getDepLevel().intValue() + 1));
     ...
     if (this.outMailFolder.getParentId().longValue() == FIRST_PARENTID)
       ...
       this.outMailFolder.setPath(parentFolder.getPath() + this.outMailFolder.getFolderId() + ".");
     }
     this.outMailFolderService.save(this.outMailFolder);
   ...
   public String remove()
   {
     ...
     if (this.folderId != null) {
       OutMailFolder tmpFolder = (OutMailFolder)this.outMailFolderService.get(new Long(this.folderId.longValue()));
 
       List<OutMailFolder> outMailFolderList = this.outMailFolderService.getFolderLikePath(tmpFolder.getPath());
       OutMailFolder folder;
       if ((count != null) && (new Long(count).longValue() > 0L)) {
         OutMailFolder deleteFolder = (OutMailFolder)this.outMailFolderService.get(new Long(FOLDER_ID_DELETE));
         for (Iterator localIterator1 = outMailFolderList.iterator(); localIterator1.hasNext(); ) { folder = (OutMailFolder)localIterator1.next();
           List<OutMail> outMailList = this.outMailService.findByFolderId(folder.getFolderId());
       ...
       for (OutMailFolder folder1 : outMailFolderList) {
         this.outMailFolderService.remove(folder1.getFolderId());
       }
     }
   ...
   public String count()
   {
     ...
     for (OutMailFolder folder : outMailFolderList) {
       Long count = this.outMailService.CountByFolderId(folder.getFolderId());
