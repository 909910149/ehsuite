...
/*  63 */     filter.addFilter("Q_userId_L_EQ", ContextUtil.getCurrentUserId().toString());
...
/*  69 */     serializer.transform(new DateTransformer("yyyy-MM-dd"), new String[] { "subAppUser.accessionTime" });
/*  70 */     buff.append(serializer.exclude(new String[] { "subAppUser.password", "class" }).serialize(list));
/*  71 */     buff.append("}");
...
/*  78 */     filter.addFilter("Q_userId_L_EQ", ContextUtil.getCurrentUserId().toString());
/*  79 */     List<UserSub> list = this.userSubService.getAll(filter);
/*  80 */     StringBuffer buff = new StringBuffer("[");
/*  81 */     for (UserSub sub : list) {
/*  82 */       buff.append("['" + sub.getSubAppUser().getUserId().toString() + "','" + sub.getSubAppUser().getFullname() + "'],");
/*     */     }
/*  84 */     if (list.size() > 0) {
...
/*     */   public String get()
/*     */   {
/* 114 */     UserSub userSub = (UserSub)this.userSubService.get(this.subId);
/*     */ 
...
/* 135 */       AppUser subAppUser = (AppUser)this.appUserService.get(subUserId);
