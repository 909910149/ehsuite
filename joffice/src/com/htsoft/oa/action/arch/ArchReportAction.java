 ...
 public class ArchReportAction extends BaseAction
 {
   ...
   public String yearReportArch()
   {
     try {
       ...
       SimpleDateFormat yearToString = new SimpleDateFormat(
         "yyyy-MM-dd");
       ...
       thisYearRollTotalFilter.getPagingBean().setStart(Integer.valueOf(start));
       ...
       thisYearFileTotalFilter.getPagingBean().setStart(Integer.valueOf(start));
   ...
   public String yearReportFile() {
     try {
       ...
       List<Dictionary> itemList = this.dictionaryService.getByItemName(this.itemName);
       ...
       for (ArchFond fond : fondList) {
         ...
         for (Dictionary d : itemList)
         {
           ...
           thisYearFileTotalFilter
             .addFilter("Q_createTime_D_GE", yearStartStr);
           thisYearFileTotalFilter.addFilter("Q_createTime_D_LE", yearEndStr);
           thisYearFileTotalFilter.addFilter("Q_timeLimit_S_LK", d.getItemValue());
           thisYearFileTotalFilter.addFilter("Q_archRoll.archFondId_L_EQ", fond.getArchFondId().toString());
           this.rollFileService.getAll(thisYearFileTotalFilter);
           int thisYearAndDicFileTotal = thisYearFileTotalFilter.getPagingBean()
             .getTotalItems();
           fond_Map.put(d.getDicId().toString(), Integer.valueOf(thisYearAndDicFileTotal));
         }
     ...
     try
     {
       ...
       for (ArchFond fond : fondList) {
         ...
         for (Dictionary d : itemList)
         {
           ...
           thisYearFileTotalFilter.addFilter("Q_tidyTime_D_LE", yearEndStr);
           thisYearFileTotalFilter.addFilter("Q_timeLimit_S_LK", d.getItemValue());
           thisYearFileTotalFilter.addFilter("Q_archRoll.archFondId_L_EQ", fond.getArchFondId().toString());
           thisYearFileTotalFilter.addFilter("Q_archStatus_SN_EQ", "1");
 
           this.rollFileService.getAll(thisYearFileTotalFilter);
           int thisYearAndDicFileTotal = thisYearFileTotalFilter.getPagingBean()
             .getTotalItems();
           fond_Map.put(d.getDicId().toString(), Integer.valueOf(thisYearAndDicFileTotal));
           if (thisYearAndDicFileTotal <= 0) continue; add = true;
         }
   ...
   public String yearReportBorrowYear() {
     try {
       ...
       data.put("rollTotal", Integer.valueOf(rollTotal)); data.put("fileTotal", fileTotal);
   ...
   public String rollReportByTimeLimit()
   {
     ...
     ((Map)map).put("name", "其它");
     ((Map)map).put("num", Integer.valueOf(allFilter.getPagingBean().getTotalItems() - d_total));
   ...
   public String fileReportByRoll() {
     ...
     Map m = new HashMap();
     m.put("name", "其它");
     m.put("nums", Integer.valueOf(fileFilter.getPagingBean().getTotalItems()));
     m.put("isTotal", Boolean.valueOf(false));
     allList.add(m);
   ...
   public String fileReportByTimeLimit() {
     ...
     Object map = new HashMap();
     ((Map)map).put("name", "其它");
     ((Map)map).put("nums", Integer.valueOf(allFilter.getPagingBean().getTotalItems() - d_total));
     ((Map)map).put("isTotal", Boolean.valueOf(false));
     list.add(map);
