 ...
 public class CarApplyAction extends BaseAction
 {
   ...
   public String save()
   {
     if (this.carApply.getApplyId() != null) {
       CarApply orgCarApply = (CarApply)this.carApplyService.get(this.carApply.getApplyId());
       try {
         BeanUtil.copyNotNullProperties(orgCarApply, this.carApply);
         this.carApplyService.save(orgCarApply);
//         if (orgCarApply.getApprovalStatus().shortValue() != Car.PASS_APPLY) 
//				  break label160; 
//				  Long receiveId = orgCarApply.getUserId();
//         Car car = (Car)this.carService.get(orgCarApply.getCar().getCarId());
...
//         this.shortMessageService.save(AppUser.SYSTEM_USER, receiveId.toString(), content, ShortMessage.MSG_TYPE_SYS);
     ...
     label160: setJsonString("{success:true}");
     return "success";
   }
 }
