			...
			initTool : function() {
				this.tools = [{
					...
					handler : function(f, d, e) {
						Ext.Msg.confirm("提示信息", "确认删除此模块吧？", function(a) {
									if (a == "yes") {
										e.ownerCt.remove(e, true);
									}
								});
					}
				}];
...
MessagePanelView = Ext.extend(Ext.ux.Portlet, {
			...
			initTool : function() {
				this.tools = [{
					...
					handler : function() {
						Ext
								.getCmp("MessagePanelView")
								.getUpdater()
								.update(__ctxPath + "/info/displayInMessage.do");
					}
				}];
			}
		});
TaskPanelView = Ext.extend(Ext.ux.Portlet, {
			...
			initTool : function() {
				this.tools = [{
					id : "refresh",
					handler : function() {
						Ext.getCmp("TaskPanelView").getUpdater()
								.update(__ctxPath + "/flow/displayTask.do");
					}
				}];
			}
		});
