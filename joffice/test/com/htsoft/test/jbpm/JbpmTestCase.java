...
/*     */   public void deploy()
/*     */   {
/*  59 */     String id = this.repositoryService.createDeployment().addResourceFromClasspath("com/htsoft/test/jbpm/test.jpdl.xml").deploy();
/*  60 */     System.out.println("deployId:" + id);
/*     */   }
/*     */ 
/*     */   public void deploySignAll() {
/*  64 */     String id = this.repositoryService.createDeployment().addResourceFromClasspath("com/htsoft/test/jbpm/signAll2.jpdl.xml").deploy();
...
/*  73 */     EnvironmentImpl env = environmentFactory.openEnvironment();
/*     */     try {
/*  75 */       ProcessDefinitionImpl pd = task.getProcessInstance().getProcessDefinition();
...
/* 102 */     EnvironmentImpl env = environmentFactory.openEnvironment();
/*     */     try
/*     */     {
/* 105 */       deployId = task.getProcessInstance().getProcessDefinition().getDeploymentId();
/*     */ 
...
/*     */     }
/*     */ 
/* 114 */     ProDefinition pd = this.proDefinitionService.getByDeployId(deployId);
/*     */ 
...
/*     */ 
/* 123 */     Element taskEl = (Element)rootEl.selectSingleNode("/process/task[@name='" + task.getActivityName() + "']");
/*     */ 
/* 125 */     if (taskEl != null) {
/* 126 */       List trans = taskEl.selectNodes("./transition");
/* 127 */       for (int i = 0; i < trans.size(); i++) {
/* 128 */         Element el = (Element)trans.get(i);
/* 129 */         System.out.println("transiton name:" + el.attributeValue("name"));
/*     */       }
/*     */     }
/*     */   }
/*     */ 
...
/*     */ 
/* 252 */     List<Task> userTaskList = this.taskService.findPersonalTasks("3");
/*     */ 
/* 254 */     for (Task tk : userTaskList)
/*     */     {
/* 256 */       System.out.println("userid 1:  task:" + tk.getName() + " taskId:" + tk.getId());
/*     */     }
/*     */ 
/* 266 */     List<Task> userTaskList6 = this.taskService.findPersonalTasks("6");
/*     */ 
/* 268 */     for (Task tk : userTaskList6)
/*     */     {
/* 270 */       System.out.println("userid 6:  task:" + tk.getName() + " taskId:" + tk.getId());
/*     */ 
/* 272 */       TaskImpl tm = (TaskImpl)this.taskService.getTask(tk.getId());
/*     */ 
/* 274 */       if (tm != null)
...
/*     */       }
/* 279 */       List transitions = this.jbpmService.getTransitionsByTaskId(tm.getId());
/*     */ 
...
/* 293 */     this.processEngine.getRepositoryService().deleteDeployment("2");
...
/*     */   {
/* 307 */     List list = this.taskDao.getByActivityNameVarKeyLongVal("发文分发", "archives.archivesId", Long.valueOf(12L));
/*     */ 
/* 309 */     System.out.println("size:" + list.size());
/*     */   }
/*     */ 
/*     */   public void getTaskImpl() {
/* 313 */     String piId = "250006";
/*     */   }
/*     */ }
